...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
     ...
     * @param requiredType  the required type of the bean to retrieve
     * @param args          arguments to use when creating a bean instance using explicit arguments
     *                      (only applied when creating a new instance as opposed to retrieving an existing one)  创建实例时使用参数
     * @param typeCheckOnly whether the instance is obtained for a type check,   是否获得实例用于类型检查，而不是实际使用
     *                      not for actual use
     * @return an instance of the bean
     * @throws BeansException if the bean could not be created
     */
//    检查Bean缓存，已经有缓存的Bean对象(有可能是半成品)则交给getObjectForBeanInstance方法处理；否则先根据Bean的生命周期类型分别实例化，
//    每种情况大致都分两步,第一步都交给createBean方法生产一个半成品的bean对象，然后同样是将半成品的bean交给getObjectForBeanInstance方法处理

    @SuppressWarnings("unchecked")
    protected <T> T doGetBean(final String name, final Class<T> requiredType, final Object[] args, boolean typeCheckOnly)
            throws BeansException {

        final String beanName = transformedBeanName(name);//规范的bean名称，beanname处理，去除FactoryBean前缀"&"等
        Object bean;
...
//将bean加入“正在创建”的集合，完成后会remove,对应afterSingletonCreation/afterPrototypeCreation方法
     ...
     * 返回bean名称，如有必要，取出工厂解除引用前缀，并将别名解析为规范名称。
     *
    ...
    protected Object getObjectForBeanInstance(
        ...
        if (BeanFactoryUtils.isFactoryDereference(name) && !(beanInstance instanceof FactoryBean)) {//如果beanname有FACTORY_BEAN_PREFIX，即以"&"为前缀,表示是FactoryBean。但后面有说!(beanInstance instanceof FactoryBean)，beanInstance不是FactoryBean，冲突了，抛出异常。
        ...
        //如果是一个FactoryBean，我们用来创建bean实例，除非调用者的确想要引用工厂
        if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {//如果不是FactoryBean则返回实例，如果是FactoryBean且name以&为前缀，返回FactoryBean引用对象。
            return beanInstance;
        }

        //下面的过程是返回FactoryBean创建的对象
        Object object = null;
        if (mbd == null) {
